...
function main() {
  ...
  const jobs = sortJobsNewestFirst(filterJobsForCurrentSession(listJobs(workspaceRoot).map((job) => reconcileJob(workspaceRoot, job)), input));
  const runningJob = jobs.find((job) => job.status === "queued" || job.status === "running" || job.status === "cancel-pending");
