...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [--sandbox <read-only|workspace-write|danger-full-access>|--full-access|--read-only] [--network] [--name <label>] [what Codex should investigate, solve, or continue]"
...
- `--sandbox <mode>`, `--full-access`, `--read-only`, `--network`, `--no-network` and `--name <label>` are runtime flags too. Preserve them for the forwarded `task` call, but do not treat them as part of the natural-language task text.
