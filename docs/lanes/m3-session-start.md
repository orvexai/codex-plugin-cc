# Lane `m3-session-start`: SessionStart context listing jobs from other and earlier sessions (FR-10)

- **Beads:** `codex-plugin-cc-e8s.5`
- **Report items:** FR-10. **Scope in this lane:** the **SessionStart hook half** of FR-10 (item 3). `status --all-sessions`, `attach` and `adopt` are lanes `m3-reattach-lib` and `m3-reattach-send`.
- **Milestone / wave:** M3, wave 1. **Depends on:** all of M2 merged. **Runs concurrently with:** `m3-resume`, `m3-control-lib`.

## Goal

After a restart, a new Claude session does not know that Codex jobs from the previous session exist, are still running, or finished unread. Make the SessionStart hook inject a short context block naming them, within its 5s budget and without any app-server call.

## Ground rules (non-negotiable)

- **Worktree:** you work in your **own git worktree**, created by the orchestrator from branch `orvex/improvement-report` after the previous wave was merged (normally `/home/crew/workspace/codex-plugin-cc-lanes/m3-session-start` on branch `lane/m3-session-start`; use the path you were given). Paths below are relative to your worktree root, and `plugins/codex/scripts/` holds the runtime. The other lanes of your wave run at the same time in their own worktrees, and the orchestrator merges every lane of the wave afterwards. **File ownership is what keeps those merges conflict-free**, so respect it exactly.
- **Beads:** run `bd` from the main checkout: `cd /home/crew/workspace/codex-plugin-cc && bd update <id> --claim` for each bead listed above. If a bead is already claimed (another lane of the same item claimed it), skip the claim and still add your note. At the end, run `bd note <id> "<one paragraph: lane m3-session-start; what changed, tests added, npm test result, open risks>"` for each bead. **Never** run `bd close`: the orchestrator closes beads after review.
...
- `plugins/codex/scripts/lib/session-context.mjs` (new)
- `tests/session-start.test.mjs` (new)
- `tests/fixtures/session-start/` (new directory)
...
- `hooks/hooks.json` (the SessionStart entry already exists; keep its timeout).
...
- **`lib/session-context.mjs`: `buildSessionStartContext({cwd, sessionId, source, now})`** → `{text, jobs:[{id, status, sessionId, badge, ageSec}]}` (text null when nothing to report). File-only: `listJobs` for the workspace, `reconcileJob` (file-only) for active jobs, then select jobs whose `sessionId !== sessionId` (or with no sessionId) that are **active**, or **terminal within the last 24h with `resultReadAt` unset**. Sort active first, then newest; cap at 10 lines plus `…and N more (status --all-sessions)` — do not print that hint until `status --all-sessions` exists (lane `m3-reattach-send` adds it); for now use `status <id>`. Each line: `<id> <status> <badge> <elapsed/age> — <title>; see: status <id> | result <id> | cancel <id>` (only commands that exist; `attach` is added by `m3-reattach-send` next wave). Use `formatRuntimeBadge` from `lib/render.mjs` (read-only import).
- **Hook output:** `handleSessionStart` keeps everything it does today (env exports, CLI shim refresh, broker lease from `m0-session-end`) and additionally prints `{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"<text>"}}` to stdout when there is text. Nothing is printed otherwise. Errors never fail the hook (catch, log to stderr, exit 0).
- **Same-id resume:** when the hook input has `source:"resume"` and the same `session_id` as earlier jobs, those jobs are the session's own, so they are not listed as "other"; list the session's own **active** jobs under a separate heading `Your running Codex jobs:` so a resumed session is reminded of them. When the hook input carries a new id, the old jobs appear under `Codex jobs from earlier sessions:`.
- **Budget:** under 300ms with 200 jobs on the fast path (measure in a test with a generous bound, e.g. < 2000ms on CI).
- **Record the Claude Code behaviour:** write `tests/fixtures/session-start/resume-same-id.json` and `resume-new-id.json` (hook input samples with `source:"resume"`), and a `README.md` in that directory stating what is known about whether `claude --resume`/`--continue` keeps the `session_id` (cite the Claude Code hooks documentation if you can find it offline in the repo or `node_modules`; otherwise say "unverified" and that both cases are handled).

## Implementation guide

- `session-lifecycle-hook.mjs`: `handleSessionStart` (and how it reads stdin hook input), the lease code from `m0-session-end`.
- `lib/state.mjs` (`listJobs`, `resolveStateDir`), `lib/workspace.mjs` (`resolveWorkspaceRoot`), `lib/job-liveness.mjs` (`reconcileJob`), `lib/render.mjs` (`formatRuntimeBadge`) — import only.
...
`tests/session-start.test.mjs` (write job records directly with the state API; run the hook with a JSON stdin like the existing SessionEnd tests do):
1. Session A has a running job and a completed unread job; SessionStart for session B prints additionalContext naming both ids, with status and badge.
2. A completed job that was read (`resultReadAt` set) or finished > 24h ago is not listed.
3. `source:"resume"` with A's own id lists the running job under "Your running Codex jobs" and not as another session's.
4. No jobs → no stdout.
5. The hook performs no app-server call (no fake app-server process is started; check `readFakeRpcLog` is empty if a fixture is installed) and finishes within the bound with 200 jobs.
6. The existing SessionStart behaviour (env file exports) still works.
...
- Not protocol-dependent. The hook must not use the native daemon or `codex agents` (no non-interactive listing exists; native surfaces §3).

## Acceptance checklist (report PASS/FAIL per line)

- [ ] B's SessionStart output mentions the job id of a job created in session A
- [ ] A resumed session with the same `session_id` sees its own earlier jobs as its own; with a new id, the SessionStart context still lists the old jobs (both recorded as fixtures, behaviour documented)
- [ ] Only running jobs and jobs finished within 24h and unread are listed, with id, status, sandbox badge and existing-command hints
- [ ] The hook reads state files only, makes no app-server calls, and stays within its budget with 200 jobs
