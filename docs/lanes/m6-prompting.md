# Lane `m6-prompting`: Model-agnostic `codex-prompting` skill, loaded on demand (WISH-6)

- **Beads:** `codex-plugin-cc-22z.10`
- **Report items:** WISH-6. **Scope in this lane:** all of WISH-6.
- **Milestone / wave:** M6, wave 1. **Depends on:** all of M5 merged. **Runs concurrently with:** `m6-worktree-lib`, `m6-broker-lib`, `m6-ci`, `m6-history-lib`, `m6-escalation-lib`.

## Goal

The prompting skill is titled "GPT-5.4 Prompting" and was preloaded by the forwarder on every dispatch. Rename it to a model-agnostic `codex-prompting` skill with per-model notes keyed off the `models` catalog, and make sure no agent or command preloads it.

## Ground rules (non-negotiable)

- **Worktree:** you work in your **own git worktree**, created by the orchestrator from branch `orvex/improvement-report` after the previous wave was merged (normally `/home/crew/workspace/codex-plugin-cc-lanes/m6-prompting` on branch `lane/m6-prompting`; use the path you were given). Paths below are relative to your worktree root, and `plugins/codex/scripts/` holds the runtime. The other lanes of your wave run at the same time in their own worktrees, and the orchestrator merges every lane of the wave afterwards. **File ownership is what keeps those merges conflict-free**, so respect it exactly.
- **Beads:** run `bd` from the main checkout: `cd /home/crew/workspace/codex-plugin-cc && bd update <id> --claim` for each bead listed above. If a bead is already claimed (another lane of the same item claimed it), skip the claim and still add your note. At the end, run `bd note <id> "<one paragraph: lane m6-prompting; what changed, tests added, npm test result, open risks>"` for each bead. **Never** run `bd close`: the orchestrator closes beads after review.
...
- `plugins/codex/skills/gpt-5-4-prompting/` (to be removed)
- `plugins/codex/skills/codex-prompting/` (new)
- `plugins/codex/agents/codex-rescue.md`
- `plugins/codex/agents/codex-exec.md`
- `plugins/codex/commands/rescue.md`
- `tests/commands.test.mjs` (only assertions that name the prompting skill or agent preloads)
...
- Create `skills/codex-prompting/` with the content of `skills/gpt-5-4-prompting/` (including any reference files), rewritten to be model-agnostic: frontmatter `name: codex-prompting`, a description that says when to load it (writing a prompt or brief for a Codex job), general guidance first, then a short "Per-model notes" section that tells the reader to run `models` for the current catalog and keys notes by model family rather than hard-coded versions. Remove `skills/gpt-5-4-prompting/` with plain file operations (`rm -r`), not `git rm`; the orchestrator records the deletion.
- Remove every preload (`skills:` frontmatter entries or equivalent) of the prompting skill from `agents/*.md` and `commands/*.md`; replace textual references to `gpt-5-4-prompting` with `codex-prompting` ("load on demand").
- Grep the whole plugin and `README.md` for `gpt-5-4-prompting` and `GPT-5.4`; fix references in files you own and report the rest as follow-ups.

## Implementation guide

- `tests/commands.test.mjs` may assert the old skill name or the preload; update only those assertions.
...
Add assertions to `tests/commands.test.mjs` (allowed above):
1. No agent or command frontmatter preloads `codex-prompting` or `gpt-5-4-prompting`.
2. `skills/codex-prompting/SKILL.md` exists and its `name` contains no model version (`/\d/` does not match the name).
3. `skills/gpt-5-4-prompting/` no longer exists.
...
- [ ] No agent or command preloads it
- [ ] The skill name encodes no model version
- [ ] Per-model notes are keyed off `models`
