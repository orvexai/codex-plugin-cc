# Lane `m3-reattach-lib`: All-sessions status snapshots and job adoption library (FR-10)

- **Beads:** `codex-plugin-cc-e8s.5`
- **Report items:** FR-10. **Scope in this lane:** the **library half** of FR-10 items 1-2: `buildStatusSnapshot(..., {allSessions})` in `lib/job-control.mjs` and `lib/job-adopt.mjs`. CLI and rendering are lane `m3-reattach-send` (wave 3).
- **Milestone / wave:** M3, wave 2. **Depends on:** wave 1 merged. **Runs concurrently with:** `m3-control`.

## Goal

Id-based access already works across sessions, but nothing lists other sessions' jobs or lets a session take a job over. Build the data layer for `status --all-sessions`, `attach` and `adopt`: an all-sessions snapshot with owner, liveness and session information, and an adoption primitive that reassigns a job's session and takes its owner lease atomically.

## Ground rules (non-negotiable)

- **Worktree:** you work in your **own git worktree**, created by the orchestrator from branch `orvex/improvement-report` after the previous wave was merged (normally `/home/crew/workspace/codex-plugin-cc-lanes/m3-reattach-lib` on branch `lane/m3-reattach-lib`; use the path you were given). Paths below are relative to your worktree root, and `plugins/codex/scripts/` holds the runtime. The other lanes of your wave run at the same time in their own worktrees, and the orchestrator merges every lane of the wave afterwards. **File ownership is what keeps those merges conflict-free**, so respect it exactly.
...
- `plugins/codex/scripts/lib/job-adopt.mjs` (new)
- `tests/reattach-lib.test.mjs` (new)

## Do not touch

- Lane `m3-control` runs at the same time and owns: `plugins/codex/scripts/codex-companion.mjs`, `plugins/codex/scripts/lib/codex.mjs`, `plugins/codex/scripts/lib/tracked-jobs.mjs`, `plugins/codex/scripts/lib/control-channel.mjs`, `plugins/codex/scripts/lib/job-budget.mjs`, `plugins/codex/scripts/lib/render.mjs`, `plugins/codex/commands/interrupt.md`, `plugins/codex/skills/codex-drive/SKILL.md`, `tests/commands.test.mjs`, `tests/fake-codex-fixture.mjs`, `tests/helpers.mjs`, `tests/interrupt.test.mjs`, `tests/budget.test.mjs`.
...
- **`buildStatusSnapshot(cwd, {allSessions = false, ...existing})`:** with `allSessions`, list every job in the workspace (no session filter), each row adding `sessionId`, `isCurrentSession`, `owner` (kind, pid), `ownerAlive`, `orphaned`, `heartbeatAgeSec`, `reconciled` (reconcile active jobs file-only first). Group the result as `{current:[…], otherSessions:[…]}` in addition to the flat list, and keep the existing output identical when `allSessions` is false.
- **`lib/job-adopt.mjs`:** `adoptJob(workspaceRoot, jobRef, {sessionId, ownerKind = "attach"|"pid"|"detached", ownerPid, onOwnerExit})` → the updated job. Under the state lock: resolve the job (any workspace, plan D6 resolution), append the old `sessionId` to `previousSessionIds[]`, set `sessionId`, and for an **active** job replace `owner` with `{kind: ownerKind, pid: ownerPid, startTime: readProcessStartTime(ownerPid), heartbeatAt: now, ttlMs}` and `onOwnerExit` (default `"cancel"` for `attach`/`pid`, `"continue"` for `detached`); write the job file and the index. For a terminal job, only the session changes. Returns `{job, previousOwner}`. `releaseOwnership(workspaceRoot, jobId)` → owner kind `detached`, `onOwnerExit:"continue"` (reuse the helper `m0-ownership` wrote for `--timeout-ms`, if it exists, instead of duplicating it). The worker's owner check (from `m0-ownership`) then follows the new owner automatically; verify that it re-reads `owner` from the job file rather than caching it, and report it as a follow-up if it caches.

## Implementation guide

- `lib/job-control.mjs`: `buildStatusSnapshot`, `filterJobsForCurrentSession`, `enrichJob`, the owner/ownerAlive fields from `m0-ownership`. `lib/state.mjs`: `withStateLock`, `upsertJob`, `writeJobFile` (import only). `lib/process.mjs`: `readProcessStartTime`.
...
**You do not own the fixture files in this wave** (lane `m3-control` does). Do not edit `tests/fake-codex-fixture.mjs` or `tests/helpers.mjs`. If you need something the fixture lacks, write a small local helper inside your own test file, or record the gap as a follow-up.

## Tests to write first

`tests/reattach-lib.test.mjs`:
1. Jobs from sessions A and B: `allSessions` lists both with `sessionId` and `isCurrentSession`; the default snapshot is unchanged (compare with the pre-change shape).
2. `adoptJob` on an active job changes `sessionId`, records `previousSessionIds`, and sets the owner to the given pid with a start time.
3. `adoptJob` on a completed job changes only the session.
4. After adoption, a status snapshot for the new session includes the job; the old session's no longer does.
5. With a real fixture worker: adopt with `ownerPid` of a `spawnStubborn()` process, kill that process, and the worker self-cancels with `cancelReason:"owner-lost"` (proves the worker follows the new owner).
...
- [ ] The all-sessions snapshot lists every workspace job with `sessionId`, owner and lease state, and liveness; the default snapshot is unchanged
- [ ] `adoptJob` sets `job.sessionId` to the adopting session and takes the owner lease (BUG-1) for active jobs
- [ ] A worker follows an adopted owner (owner loss after adoption cancels with `owner-lost`)
